...
def get_week_phase():
    ...
    after_draw = (wd == 4 and (now.hour, now.minute) >= (19, 30))
    # 表示用：土→日→月→火→水→木→金（金19:30リセット）
    days = [("土", "結果・分析", 5), ("日", "積み上げ", 6), ("月", "積み上げ", 0),
            ("火", "積み上げ", 1), ("水", "積み上げ", 2), ("木", "最終チェック", 3), ("金", "決定・購入", 4)]
    missions = {
        5: ("結果発表 ＆ 辛口分析", "前回の結果を取り込んで自動採点し、忖度なしの総監督レポートで先週を総括します。", "結果を取得して採点する", "最新データ取得"),
        6: ("今週のサイクル開始", "今日の気持ちを言葉にして、最初の予測を積み上げましょう。", "予測を積み上げる", "日々の予想・積上げ"),
    ...
    if after_draw:
        mission = ("抽選完了。明朝、結果と反省へ", "今夜の抽選でサイクルはリセット。明日の朝、結果を取り込んで振り返りましょう。", "結果と振り返りへ", "結果発表と振り返り")
    else:
        mission = missions[wd]
...
if st.session_state.menu == "ホーム":
    ...
    st.markdown(f"<div class='week'>{nodes}</div><div class='reset-note'>― 金 19:30 抽選＝リセット → 翌週へ ―</div>", unsafe_allow_html=True)
